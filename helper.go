...
// If componentsDst is provided with sufficient capacity, it will be used
// instead of allocating a new slice.
func parseObjectIdentifier(src []byte, componentsDst []uint32) (string, []uint32, error) {
	if len(src) == 0 {
	...
	// Use provided slice if it has capacity, otherwise allocate
	var components []uint32
	if cap(componentsDst) >= len(src) {
		components = componentsDst[:0]
	} else {
...
// If componentsDst is provided with sufficient capacity, it will be used for components.
func parseRawOID(logger Logger, data []byte, componentsDst []uint32) (string, []uint32, int, error) {
	if len(data) == 0 {
	...
	oid, components, err := parseObjectIdentifier(data[cursor:length], componentsDst)
	if err != nil {
		return "", nil, 0, err
	}
...
func parseRawField(logger Logger, data []byte, msg string) (any, int, error) {
	...
	case ObjectIdentifier:
		...
		oid, _, err := parseObjectIdentifier(data[cursor:length], nil)
