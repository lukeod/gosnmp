...
// TestSendV3TrapMD5AuthNoPrivMismatchedEngineID verifies that an authenticated
// SNMPv3 trap is accepted when the sender's AuthoritativeEngineID differs from
// the listener's. Per RFC 3414 section 1.5.1, the trap sender is the
// authoritative engine, so the listener must re-derive the localized key using
// the sender's engine ID.
func TestSendV3TrapMD5AuthNoPrivMismatchedEngineID(t *testing.T) {
	done := make(chan int)

	tl := NewTrapListener()
	defer tl.Close()

	listenerEngineID := string([]byte{0x80, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04})
	senderEngineID := string([]byte{0x80, 0x00, 0x00, 0x00, 0x0A, 0x0B, 0x0C, 0x0D})

	listenerSP := &UsmSecurityParameters{
		UserName:                 "test",
		AuthenticationProtocol:   MD5,
		AuthenticationPassphrase: "password",
		AuthoritativeEngineBoots: 1,
		AuthoritativeEngineTime:  1,
		AuthoritativeEngineID:    listenerEngineID,
	}

	senderSP := &UsmSecurityParameters{
		UserName:                 "test",
		AuthenticationProtocol:   MD5,
		AuthenticationPassphrase: "password",
		AuthoritativeEngineBoots: 1,
		AuthoritativeEngineTime:  1,
		AuthoritativeEngineID:    senderEngineID,
	}

	tl.OnNewTrap = makeTestTrapHandler(t, done, Version3)
	tl.Params = Default
	tl.Params.Version = Version3
	tl.Params.SecurityParameters = listenerSP
	tl.Params.SecurityModel = UserSecurityModel
	tl.Params.MsgFlags = AuthNoPriv
...
func TestSendV3TrapSHAAuthNoPrivMismatchedEngineID(t *testing.T) {
	done := make(chan int)
