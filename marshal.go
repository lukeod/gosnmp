...
// Request ID 0 is always considered valid (used by some REPORT PDUs).
...
func (x *GoSNMP) handleReportPDU(result, packetOut *SnmpPacket,
	...
	// Always store security parameters from REPORT
	_ = x.storeSecurityParameters(result)
	...
	case usmStatsNotInTimeWindows:
		// Recoverable via inline resend (PR #532 fix for NoAuthNoPriv REPORTs)
...
func (x *GoSNMP) receiveAndProcessResponse(packetOut *SnmpPacket, allReqIDs []uint32,
	...
	if x.Version == Version3 {
		...
		if !skipAuth {
			...
			if err := x.testAuthentication(resp, result, useResponseSP); err != nil {
...
func (x *GoSNMP) doRequestAttempt(packetOut *SnmpPacket, allReqIDs []uint32,
	...
	for {
		...
		if err := x.sendPacket(outBuf, deadline); err != nil {
