...
func (x *GoSNMP) unmarshalVBL(packet []byte, response *SnmpPacket) error {
	...
	estimatedPDUs := vblLength / 14
	if estimatedPDUs > cap(response.Variables) {
		response.Variables = make([]SnmpPDU, 0, estimatedPDUs)
	}

	// Pre-allocate backing array for all NameComponents (~10 components per OID).
	componentsBacking := make([]uint32, estimatedPDUs*10)
	componentsOffset := 0
	...
	for cursor < vblLength {
		...
		// Parse OID, using slice of backing array for components
		var componentsDst []uint32
		if componentsOffset < len(componentsBacking) {
			componentsDst = componentsBacking[componentsOffset:]
		}
		oid, oidComponents, oidLength, err := parseRawOID(x.Logger, packet[cursor:], componentsDst)
		if err != nil {
			return fmt.Errorf("error parsing OID Value: %w", err)
		}
		componentsOffset += len(oidComponents)
		cursor += oidLength
		if cursor > len(packet) {
