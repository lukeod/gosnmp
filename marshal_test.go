...
func TestSNMPv3_UnknownEngineIDReport_EngineIDMismatchPolicy(t *testing.T) {
	// When REPORT engineID mismatches an already-known session
	// engineID, do not adopt it; retry should keep using the original engineID.
	oldEngineID := mustHex("80001f88801cae0f4e0000001d")
	newEngineID := mustHex("80001f88801cae0f4e0000001e")

	// Start from captured v3 REPORT payload and mutate:
	// - usmStatsNotInTimeWindows -> usmStatsUnknownEngineIDs
	// - engineID old -> engineID new
	baseReport := mustHex(
		"306d020103301002040c6e214f020205c004010002010304243022040d80001f88801cae0f4e0000001d020101020310b1b9040561646d696e040004003030040d80001f88801cae0f4e0000001d0400a81d02010002010002010030123010060a2b060106030f0101020041021e5d",
	)
	oldReportOID := mustHex("2b060106030f01010200")
	newReportOID := mustHex("2b060106030f01010400")

	report := bytes.ReplaceAll(baseReport, oldReportOID, newReportOID)
	report = bytes.ReplaceAll(report, oldEngineID, newEngineID)

	ap := startMockUniFiAP(t, report)
	...
	g := &GoSNMP{
		...
		SecurityParameters: &UsmSecurityParameters{
			...
			AuthenticationProtocol:   SHA,
			AuthenticationPassphrase: "pass",
			AuthoritativeEngineID:    string(oldEngineID),
			AuthoritativeEngineBoots: 1,
			AuthoritativeEngineTime:  207512,
		},
	}
	require.NoError(t, g.Connect(), "Connect")
	t.Cleanup(func() { _ = g.Conn.Close() })
	...
	firstWrite := ap.FirstWrite()
	secondWrite := ap.SecondWrite()
	require.NotEmpty(t, firstWrite, "expected initial client datagram")
	require.NotEmpty(t, secondWrite, "expected resend after unknownEngineID REPORT")

	require.True(t, bytes.Contains(firstWrite, oldEngineID), "first request should use original engineID")
	require.True(t, bytes.Contains(secondWrite, oldEngineID), "resend should keep original engineID on mismatch")
	require.False(t, bytes.Contains(secondWrite, newEngineID), "resend should not adopt mismatched REPORT engineID")

	usp, ok := g.SecurityParameters.(*UsmSecurityParameters)
	require.True(t, ok, "expected UsmSecurityParameters")
	assert.Equal(t, string(oldEngineID), usp.AuthoritativeEngineID, "session engineID should remain unchanged")
}
